...
static const struct asm_op_triplet ASM_OPS[] = {
  { ASM_PRI, "NUL", 0 }, // Because spec doesn't include any instruction with opcode 0
  { ASM_ADD, "ADD", 3 },
  { ASM_MUL, "MUL", 3 },
  { ASM_SOU, "SOU", 3 },
  { ASM_DIV, "DIV", 3 },
...
void asm_fprintf(FILE* out, struct asm_table* table) {
  for (int i = 0; i < table->instructions_count; i++) {
    ...
    if (ASM_OPS[instr.op].params >= 1) fprintf(out, " %d", instr.arg0);
    if (ASM_OPS[instr.op].params >= 2) fprintf(out, " %d", instr.arg1);
    if (ASM_OPS[instr.op].params >= 3) fprintf(out, " %d", instr.arg2);
    fprintf(out, "\n");
  }
}
...
uint32_t asm_append(struct asm_table* table, enum asm_op_code op, uint32_t arg0, uint32_t arg1, uint32_t arg2) {
  if (table->instructions_count >= table->instructions_size) {
    // Increase array size
    table->instructions = realloc(table->instructions, (table->instructions_size * 2) * sizeof(struct asm_instr));
