...
static void assert_params_in_bounds(struct asm_instr* instr, int count) {
  ...
  if (count >= 1 && instr->arg0 >= MEMORY_SIZE) faulty_address = instr->arg0;
  if (count >= 2 && instr->arg1 >= MEMORY_SIZE) faulty_address = instr->arg1;
  if (count >= 3 && instr->arg2 >= MEMORY_SIZE) faulty_address = instr->arg2;
